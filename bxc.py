...
def parse_args():
    ...
    PARSER = argparse.ArgumentParser(
        description='Convert values with Base X to Base Y')
    PARSER.add_argument(
        'INPUT_VALUE',
        metavar='INPUT_VALUE',
        type=str,
    ...
    PARSER.add_argument(
        ...
        dest='INPUT_BASE',
        type=int,
        help='Base of input Value')
    PARSER.add_argument(
        '-b',
        dest='OUTPUT_BASE',
        type=int,
        help='Base of output Value')
    PARSER.add_argument(
        '-O',
        dest='INPUT_ORDER',
        type=str,
        help='Order of input Alphabet '
        'e.g. "0aA" results in 0..9a..zA..Z')
    PARSER.add_argument(
        '-o',
        dest='OUTPUT_ORDER',
        type=str,
        help='Order of output '
        'Alphabet e.g. "0aA" results in 0..9a..zA..Z')
    PARSER.add_argument(
        '-A',
        dest='INPUT_ALPHABET',
        type=str,
        help='Alternative input '
        'alphabet (Will override -O)')
    PARSER.add_argument(
        '-a',
        dest='OUTPUT_ALPHABET',
        type=str,
        help='Altvernative output '
        'alphabet (Will override -o)')
    PARSER.add_argument(
        '-P',
        dest='INPUT_ALPHABET_APPEND',
        type=str,
        help='Append '
        'characters to input alphabet')
    PARSER.add_argument(
        '-p',
        dest='OUTPUT_ALPHABET_APPEND',
        type=str,
        help='Append '
...
def calculate_alphabet_from_order(ORDER):
    ...
    for CHAR in ORDER:
        if CHAR.islower():
            ...
            ALPHABET = "%s%s" % (ALPHABET, string.digits)
    return ALPHABET
...
def main():
    ...
    if INPUT_ALPHABET is None:
        if INPUT_ORDER is not None:
            ...
            INPUT_ALPHABET = string.digits + string.ascii_lowercase + string.ascii_uppercase

    if OUTPUT_ALPHABET is None:
        if OUTPUT_ORDER is not None:
            OUTPUT_ALPHABET = calculate_alphabet_from_order(OUTPUT_ORDER)
        else:
            OUTPUT_ALPHABET = string.digits + string.ascii_lowercase + string.ascii_uppercase
    ...
    if len(INPUT_ALPHABET) < INPUT_BASE:
        print(
            "ERROR: Input alphabet is shorter than base. Use -P to append additonal chars")
        ALPHABET_TOO_SHORT = True
    if len(OUTPUT_ALPHABET) < OUTPUT_BASE:
        print(
            "ERROR: Output alphabet is shorter than base. Use -p to append additonal chars")
        ALPHABET_TOO_SHORT = True
    ...
    for CHAR in INPUT_VALUE:
        if CHAR not in INPUT_ALPHABET:
            print(
                "ERROR: The character \"%s\" is not part of the input alphabet" %
                CHAR)
            sys.exit(3)
